...
if(Sys.info()[7]=="rachaelorben") {
  ...
  savedirFiles<-'Analysis/data_processed/ATN_proj/'
  savedirPlots<-'Analysis/DataViz/'
...
# deployments from folder names -------------------------------------------

ATNprj <- list.files(path=paste0(userdir,"data/ornitela_for_ATN"))

# Pulls in GPS data only & saves as RDS--------------------------------------------------
for (j in 1:length(ATNprj)){
  
  GPS_files<-list.files(path=paste0(userdir,"data/ornitela_for_ATN/",ATNprj[j],"/gps_sensors_v2"), full.names = TRUE)
  my_files <- fileSnapshot(path=paste0(userdir,"data/ornitela_for_ATN/",ATNprj[j],"/gps_sensors_v2"))
  File_sel<-GPS_files[which(my_files$info[1] > 2500)] #selects files with >309 bytes (1 header row)
  
  deply_sel<-deploy_matrix[deploy_matrix$Project_ID==ATNprj[j],]
  
  tracks<-NULL
  for(i in 1:length(File_sel)) {
    birdy<-fread(file=File_sel[i],stringsAsFactors=FALSE,sep = ",",fill=TRUE)
    birdy$datetime<-ymd_hms(birdy$UTC_datetime)
      
    deply_sel_ID<-deply_sel%>%filter(TagSerialNumber==birdy$device_id[1]) #trims to deployment start
    birdy<-birdy%>%filter(datetime>deply_sel_ID$DeploymentStartDatetime[1])
    if (is.na(deply_sel_ID$DeploymentEndDatetime_UTC)==FALSE){birdy<-birdy%>%filter(datetime<deply_sel_ID$DeploymentEndDatetime[1])}
    
    birdy$Latitude[birdy$Latitude==0]<-NA #makes 0 locations NA
    birdy$Longitude[birdy$Longitude==0]<-NA
    
    birdy$ATN_prj<-ATNprj[j]
    tracks<-rbind(tracks,birdy)
  }
  saveRDS(tracks,paste0(userdir,savedirFiles,ATNprj[j],"_gps_sensors_v2.rds"))
  }


# Read in RDS, sp filter, & plot GPS only ---------------------------------

for (j in 1:length(ATNprj)){
  Birds<-readRDS(paste0(userdir,savedirFiles,ATNprj[j],"_gps_sensors_v2.rds"))
  deply_sel<-deploy_matrix[deploy_matrix$Project_IDIDS[i],]
